...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = np.dot((y_hat - y).T, outsideVectors)
...
def negSamplingLossAndGradient(
    ...
    negSampleWordVectors = np.array([outsideVectors[i] for i in negSampleWordIndices])
    
    sigmoid_center = sigmoid(np.dot(outsideWordVec, centerWordVec))
    sigmoid_negsample = sigmoid(- np.dot(negSampleWordVectors, centerWordVec))
    
    loss = - np.log(sigmoid_center) - np.sum(np.log(sigmoid_negsample))
    
    gradCenterVec = np.sum(negSampleWordVectors * np.array([(1 - sigmoid_negsample) for i in range(len(centerWordVec))]).T, axis=0) + (sigmoid_center - 1)*outsideWordVec
    ...
    for i, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - sigmoid_negsample[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        loss += loss_
        gradCenterVecs[currentCenterWordIdx]+= gradCenterVec_
